...
static int compareGameKey(const void* key, const void* element);
static int strcasecmp_custom(const char* s1, const char* s2);

static Game* findGameLinear(const char* name);
static Game* findGameBsearch(const char* name);

static void binarySearchGenre(const char* genre, int left, int right, bool* found);
...
static void loadGamesFromFile(void) {
    ...
    if (file == NULL) {
        printf("Datoteka %s nije pronadena: %s\n", FILE_NAME, strerror(errno));
        return;
    }
...
static void addGame(void) {
    ...
    printf("Ime igre: ");
    if (!readLine(newGame.name, MAX_NAME_LENGTH)) {
    ...
    printf("Zanr: ");
    if (!readLine(newGame.genre, MAX_GENRE_LENGTH)) {
...
static void suggestByGenre(void) {
    ...
    printf("Unesi zanr: ");
    if (!readLine(genre, MAX_GENRE_LENGTH)) {
        printf("Neispravan unos zanra.\n");
        return;
    }

    printf("Preporuke za zanr %s:\n", genre);
    binarySearchGenre(genre, 0, (int)gameCount - 1, &found);
...
static void deleteGame(void) {
    ...
    if (!readLine(name, MAX_NAME_LENGTH)) {
        printf("Neispravan unos imena.\n");
        return;
    }
...
static void updateGame(void) {
    ...
    if (!readLine(foundGame->genre, MAX_GENRE_LENGTH)) {
        printf("Neispravan unos zanra.\n");
        return;
    }
...
static void insertGame(void) {
    ...
    printf("Unesi poziciju za insert od 1 do %zu: ", gameCount + 1);

    if (!readSize(&position) || position < 1 || position > gameCount + 1) {
    ...
    *copiedGame = newGame;
    ...
    for (size_t i = gameCount; i > position - 1; i--) {
        games[i] = games[i - 1];
    }
...
static void searchGame(void) {
    ...
    Game* foundGame = findGameBsearch(name);
    ...
    printf("Pronadena igra: %s [%s]\n", foundGame->name, foundGame->genre);
}

static Game* findGameLinear(const char* name) {
    if (name == NULL || games == NULL) {
        return NULL;
    }
...
static Game* findGameBsearch(const char* name) {
    if (name == NULL || games == NULL || IS_EMPTY(gameCount)) {
        return NULL;
    }

    qsort(games, gameCount, sizeof(Game), compareGames);

    return bsearch(name, games, gameCount, sizeof(Game), compareGameKey);
}

static void binarySearchGenre(const char* genre, int left, int right, bool* found) {
    if (genre == NULL || found == NULL || games == NULL || left > right) {
        return;
    }

    int mid = left + (right - left) / 2;

    binarySearchGenre(genre, left, mid - 1, found);
    ...
    binarySearchGenre(genre, mid + 1, right, found);
}
...
static bool resizeGames(size_t newCount) {
    ...
    Game* tempPtr = realloc(games, newCount * sizeof(Game));
...
static bool readSize(size_t* value) {
    ...
    if (scanf("%zu", &number.sizeValue) != 1) {
        clearInputBuffer();
        return false;
    }
...
static int compareGames(const void* a, const void* b) {
    ...
    const Game* firstGame = (const Game*)a;
    const Game* secondGame = (const Game*)b;

    return strcasecmp_custom(firstGame->name, secondGame->name);
}
...
static int compareGameKey(const void* key, const void* element) {
    ...
    const Game* game = (const Game*)element;

    return strcasecmp_custom(name, game->name);
}
...
static int strcasecmp_custom(const char* s1, const char* s2) {
    ...
    while (*s1 && *s2) {
        int first = tolower((unsigned char)*s1);
        int second = tolower((unsigned char)*s2);
